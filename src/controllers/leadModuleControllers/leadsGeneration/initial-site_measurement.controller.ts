...
export class PaymentUploadController {
  ...
  public getISMPaymentInfoByLeadId = async (req: Request, res: Response): Promise<void> => {
    try {
      const { leadId } = req.params;
      ...
      const result = await this.paymentUploadService.getISMPaymentInfoByLeadId(parseInt(leadId));
