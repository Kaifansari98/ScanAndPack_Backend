...
const getVendorId = (req: Request): number | null => {
  const id = Number(req.params.vendor_id ?? req.query.vendor_id ?? req.body.vendor_id);
  return isNaN(id) || id <= 0 ? null : id;
};

const getUserId = (req: Request): number | null => {
  const id = Number(
    (req as any).user?.id ?? (req as any).user?.user_id ??
...
function parseVendors(vendors: any[]): any[] | null {
  for (const v of vendors) {
    if (!v.company_vendor_id || !v.required_qty || Number(v.required_qty) <= 0) {
      return null;
    }
  }
  ...
  return vendors.map((v) => ({
    ...
    estimated_price:
      v.estimated_price != null
        ? Number(v.estimated_price)
        : undefined,
...
export const create = async (req: Request, res: Response) => {
  ...
  for (const item of items) {
    if (!item.product_id)
      return res.status(400).json(ApiResponse.error("Each item needs product_id", 400));
    const parsed = parseVendors(item.vendors ?? []);
    if (parsed === null)
      return res.status(400).json(ApiResponse.error("Each vendor entry needs company_vendor_id and required_qty > 0", 400));
    item.vendors = parsed;
  }

  const result = await createPurchaseIntent({ vendor_id, user_id, category_id, priority, remarks, items });
...
export const patchStatus = async (req: Request, res: Response) => {
  ...
  if (!valid.includes(status))
    return res.status(400).json(ApiResponse.error(`Invalid status. Valid: ${valid.join(", ")}`, 400));
...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0)
      return res.status(400).json(ApiResponse.error("At least one item is required", 400));

    for (const item of items) {
      if (!item.product_id)
        return res.status(400).json(ApiResponse.error("Each item must have a product_id", 400));
      if (!Array.isArray(item.vendors))
        return res.status(400).json(ApiResponse.error("Each item must have a vendors array", 400));
      const parsed = parseVendors(item.vendors);
      if (parsed === null)
        return res.status(400).json(ApiResponse.error(
          `Each vendor entry needs company_vendor_id and required_qty > 0 (product_id: ${item.product_id})`, 400
        ));
      item.vendors = parsed;
    }

    const result = await updatePurchaseIntentService(id, vendor_id, user_id, {
      category_id, priority, remarks, items,
    });
