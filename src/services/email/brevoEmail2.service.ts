...
export const sendFinalMeasurementAssignedEmail = async (payload: {
  ...
  const defaultSubject = `Final Measurement Assigned for ${payload.leadCode} - ${payload.leadName}`;

  const defaultText = [
    `Hello ${payload.toName ?? "there"},`,
    "",
    "You have been assigned the Final Measurement task for the following lead.",
    "",
    `Lead Code: ${payload.leadCode}`,
    `Lead Name: ${payload.leadName}`,
    `Contact Details: ${payload.contactNumber}`,
    `Assigned By: ${payload.assignedBy}`,
    `Assigned Date: ${payload.assignedAt}`,
    "",
    "Please schedule the site visit and update the final measurements in the system.",
    "",
    ...
    <div style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:10px;padding:20px;">
      ...
      <div style="border:1px solid #e5e7eb;border-radius:8px;padding:12px;background:#f8fafc;">
        ...
        <div class="lead-info-row">
          ...
          <div class="lead-info-value">${payload.contactNumber}</div>
        </div>
        <div class="lead-info-row">
        ...
        payload.projectUrl
          ? `<div style="margin:16px 0 0;text-align:start;">
              <a
                ...
                text-decoration:none;padding:10px 16px;border-radius:6px;font-size:14px;"
                target="_blank"
                rel="noopener noreferrer"
              >
                View Lead Details
...
`;
  ...
  const template = await prisma.emailNotificationMaster.findFirst({
    where: {
      vendor_id: payload.vendor_id,
      template_key: "FINAL_MEASUREMENT_ASSIGNED",
      active: true,
    },
  });
...
// 2
export const sendSiteReadinessAssignedEmail = async (payload: {
  vendor_id: number;
  toEmail: string;
  toName?: string;
  ...
  const defaultSubject = `Site Readiness Check Assigned for ${payload.leadCode} - ${payload.leadName}`;

  const defaultText = [
    `Hello ${payload.toName ?? "there"},`,
    "",
    "You have been assigned the Site Readiness task for the following lead.",
    "",
    `Lead Code: ${payload.leadCode}`,
    `Lead Name: ${payload.leadName}`,
    `Contact Details: ${payload.contactNumber}`,
    ...
    `Assigned Date: ${payload.assignedAt}`,
    "",
    "Please verify site conditions and update the readiness status accordingly.",
    "",
    payload.projectUrl ? `View Lead Details: ${payload.projectUrl}` : "",
  ]
    .filter(Boolean)
...
`;
  ...
  const template = await prisma.emailNotificationMaster.findFirst({
    where: {
      vendor_id: payload.vendor_id,
      template_key: "SITE_READINESS_ASSIGNED",
      active: true,
    },
  });
