...
export const validateFiles = async (
  ...
  try {
    ...
    if (!files?.upload_pdf || files.upload_pdf.length === 0) {
      ...
      const hasAnyMeasurementDoc = existingDocs.some(
        (doc) => doc.doc_type_id === measurementDocType.id,
      );

      if (!hasAnyMeasurementDoc) {
        return res.status(400).json({
          success: false,
          message: "At least one document file upload is mandatory",
        });
...
export const handleMulterError = (
  ...
  if (err) {
    ...
    if (
      err.message === "Invalid file type. Only PDF and image files are allowed."
    ) {
      return res.status(400).json({
        success: false,
        message: err.message,
